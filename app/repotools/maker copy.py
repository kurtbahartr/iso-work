...
def mkinitcpio(project):
    try:
        ...
        print("mkinitcpio" in project.all_packages)
        # print(rep.packages.keys())
        if "mkinitcpio" in project.all_packages:
            prog = "mkinitcpio"
        ...
        elif "mkinitramfs" in project.all_packages:
            prog = "mkinitramfs"
    ...
    if os.path.exists(sddmconf_path):
        ...
        for line in open(sddmconf_path, "r").readlines():
            if line.startswith("User="):
                ...
                lines.append("Session=plasmax11.desktop\n")
            # elif line.startswith("#ServerTimeout="):
            #    lines.append("ServerTimeout=60\n")
            else:
                lines.append(line)
...
def install_packages(project):
    ...
    if 'Calamares' in project.all_packages:
        ...
        packages = project.all_packages  # or repo.full_deps("yali")
...
def squash_live_config_image(project):
    ...
    if "yali" not in project.all_packages:
        packages = ["yali"]
    ...
    if project.type == "install":
        if "xdm" in project.all_packages:
    ...
    if project.type == "live":
        ...
        shutil.copy("./data/etc/timezone", "{}/etc/timezone".format(config_image_dir))
        ...
        # kde yapılandırması ================================================
        if 'plasma-workspace' in project.all_packages:
            ...
            os.system("cp -rf ./data/kde_conf/usr/share/wallpapers {}/usr/share".format(config_image_dir))
            
            os.system("cp -rf ./data/etc {}/etc".format(config_image_dir))
...
def squash_image(project):
    ...
    if project.type == "live":
        ...
        if 'xfce4-panel' in project.all_packages:
            run("mkdir -p {}/root/Desktop".format(image_dir))
            shutil.copy("./data/yali/yali.desktop", "{}/root/Desktop/".format(image_dir))
            shutil.copy("./data/yali/yali-rescue.desktop", "{}/root/Desktop/".format(image_dir))

        if 'kservice' in project.all_packages:
            # masaüstüne yalı simgelerini ekliyoruz
        ...
        # lxqt
        #run("mkdir -p {}/home/pisi/.config/lxqt".format(image_dir))
        ...
        # sddm
        #run("mkdir -p {}/usr/lib/sddm/sddm.conf.d".format(image_dir))
        #shutil.copy("./data/kde_conf/sddm.conf.d/sddm.conf", "{}/usr/lib/sddm/sddm.conf.d/".format(image_dir))
        ...
        if 'plasma-workspace' in project.all_packages:
            ...
            os.system("cp -rf ./data/kde_conf/sddm.conf.d/sddm.conf", "{}/usr/lib/sddm/sddm.conf.d/".format(image_dir))
        ...
        if 'lxqt-session' in project.all_packages:
            run("mkdir -p {}/home/pisi/.config/lxqt".format(image_dir))
            shutil.copy("./data/lxqt/session.conf", "{}/home/pisi/.config/lxqt/".format(image_dir))
            shutil.copy("./data/lxqt/sddm.conf", "{}/usr/lib/sddm/sddm.conf.d/".format(image_dir))
...
def make_repos(project):
    ...
    try:
        ...
        if project.type == "install":
            imagedeps = project.all_packages or \
                repo.full_deps("yali")
...
def make_image(project):
    ...
    try:
        ...
        obj.setUser(0, "", "", "", "root", "", dbus_interface="tr.org.pardus.comar.User.Manager")
...
def make_EFI(project, grub=True):
    ...
    if grub:
        ...
        run("cp -rf ./data/49-livecd.rules %s/usr/share/polkit-1/rules.d" % image_dir)
