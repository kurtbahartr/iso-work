...
+        run('chroot "{}" {}'.format(image_dir, command), ignore_error=True)
-    def chrun(cmd):
-        run('chroot "%s" %s' % (image_dir, cmd), ignore_error=True)
 
+    def copy_to_skel(source, destination):
+        target_dir = os.path.join(image_dir, 'etc', 'skel', destination)
...
-    if os.path.exists("{}/etc/yali/".format(image_dir)):
         shutil.copy("./data/yali/repos.json",
+                    os.path.join(image_dir, 'etc', 'yali'))
-                "{}/etc/yali/".format(image_dir))
...
+            chroot_command("chown -R pisi:wheel /home/pisi/.config")
+            chroot_command("chown -R pisi:wheel /home/pisi/.local")
...
+    def copy_display_manager_config():
+        repo = project.get_repo()
...
-            #   ignore_error=True)
     ...
     if project.type == "install":
+        run("rm -rf {}/run/livemedia".format(image_dir))
+        run("rm -rf {}/home/pisi/.livemedia".format(image_dir))
-        if os.path.exists("%s/run/livemedia" % image_dir):
-            run("rm %s/run/livemedia" % image_dir)
-
-        if os.path.exists("%s/home/pisi/.livemedia" % image_dir):
...
+        run("mkdir -p {}/run/pisilinux".format(image_dir))
+        run("touch {}/run/pisilinux/livemedia".format(image_dir))
+        run("touch {}/home/pisi/.livemedia".format(image_dir))
...
+    exclude_list_file = tempfile.NamedTemporaryFile()
+    with open(exclude_list_file.name, "w") as file:
...
+    mksquashfs_command = 'mksquashfs "{}" "{}" -noappend -comp {} -ef "{}"'.format(
+        image_dir, image_file, project.squashfs_comp_type, exclude_list_file.name)
+    run(mksquashfs_command)
+    with open(os.path.join(project.work_dir, "finished.txt"), 'w') as file:
+        file.write("pack-live")
...
<<<<<<<  ed85aa90-8df7-4cf5-a15a-379c7271bc73  >>>>>>>
