...
def mkinitcpio(project):
    try:
        ...
        kernel_version = rep.packages['kernel-next'].version

        # chrun(" ".join([binary, "-k", kernel_version,
                       # "-c %s" % config_path, extra_args]))
        chrun(" ".join([binary, "-k", "6.12.31-next", # Bu satırı manuel olarak değiştirin
                             "-c %s" % config_path, extra_args]))
...
def setup_isolinux(project):
    ...
    kernel_version = open(os.path.join(image_dir, "etc/kernel/kernel-next")).read()
...
def setup_live_gdm(project):
    ...
    try:
        # Dizin yoksa oluştur (Python 2 uyumlu)
        dir_path = os.path.dirname(gdmconf_path)
        if not os.path.exists(dir_path):
            os.makedirs(dir_path)

        lines = []
        daemon_section = False
        automatic_added = False
        ...
        for line in lines:
            if line.strip().startswith("[daemon]"):
                ...
                # Otomatik giriş ayarlarını ekle
                new_lines.append("AutomaticLoginEnable=True\n")
                new_lines.append("AutomaticLogin=pisi\n")
                new_lines.append("WaylandEnable=false\n")
                automatic_added = True
            elif line.strip().startswith("[") and daemon_section:
                # [daemon] bölümünden çıkınca ekleme yapma
                daemon_section = False
                new_lines.append(line)
            else:
                new_lines.append(line)

        # [daemon] bölümü yoksa en başa ekle
        if not automatic_added:
            new_lines.insert(0, "[daemon]\nAutomaticLoginEnable=True\nAutomaticLogin=pisi\nWaylandEnable=false\n\n")

        # Dosyayı yaz
        with open(gdmconf_path, "w") as f:
            f.writelines(new_lines)

        # İzinleri ayarla (Python 2 octal syntax)
        os.chmod(gdmconf_path, 0644)

        print "*** GDM custom.conf basariyla guncellendi"
...
def squash_live_config_image(project):
    ...
    if project.type == "live":
        ...
        repo = project.get_repo()
        # kernel_version = repo.packages['kernel-next'].version
        ...
        # os.system("cp -rf %s/%s %s/var/cache/pisi/packages/" % (repo.cache_dir, repo.packages['kernel-next'].uri, config_image_dir))
...
def squash_image(project):
    ...
    if project.type == "live":
        ...
        # sddm
        run("mkdir -p %s/usr/lib/sddm/sddm.conf.d" % image_dir)
        shutil.copy("./data/lxqt/sddm.conf",
                    "{}/usr/lib/sddm/sddm.conf.d/".format(image_dir))
        ...
        # os.system("cp -rf %s/%s %s/var/cache/pisi/packages/" % (repo.cache_dir, repo.packages['kernel-next'].uri, image_dir))
